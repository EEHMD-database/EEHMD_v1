...
//void callback(const ros_eehmd::leaprosConstPtr& leap_msg, const ros_eehmd::EmgArrayConstPtr& myo_msg, const ros_eehmd::ImuConstPtr& imu_msg)

void callback(const ros_eehmd::leapmsgConstPtr& leap_msg, const ros_eehmd::EmgArrayConstPtr& myo_msg)
{
  if (leap_msg->hands_count > 0){
    bool flag = true;
    hand.compute_model_from_leap_msg(*leap_msg);
    ros_eehmd::JointCoordinates joint_coordinates;
